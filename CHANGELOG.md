...
- **Clipboard-Listener und State bereinigt** - Der `mfr:clipboard-updated`-Listener wird jetzt zentral registriert und nicht mehr pro Instanz am `document` gehalten. Zusätzlich ist der Clipboard-State pro Repeater-Instanz abgeschottet, damit PJAX-/Reload-Szenarien keine alten Instanzen oder fremde Repeater mehr mitziehen.
- **Paste-at-start gegen fehlendes Template abgesichert** - Beim Einfügen am Anfang wird der Fokus jetzt nur gesetzt, wenn `_renderItem()` tatsächlich ein Element geliefert hat. Damit bleibt der Start-Paste-Pfad auch bei fehlendem Template stabil.
