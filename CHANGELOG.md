...
- **Dokumentation: Code-Snippet-Kopieren funktioniert auch ohne Clipboard API** – Der Copy-Button in der Doku (`assets/js/docs.js`) nutzt jetzt einen robusten Fallback via `document.execCommand('copy')`, wenn `navigator.clipboard.writeText(...)` nicht verfügbar ist oder fehlschlägt (z. B. in nicht-sicheren Kontexten).
