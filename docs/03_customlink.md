...
- Im Flex-Repeater sollte `addCustomLinkField()` **immer mit einem String** als ID verwendet werden (z. B. `'link'`). Damit landet der Wert unter dem lesbaren Schlüssel `$item['link']` im JSON. Eine numerische ID funktioniert zwar technisch, ergibt aber den wenig hilfreichen Schlüssel `$item['6']`.
...
### Ältere Repeater-Werte (`{name, id}`-Arrays)

Frühere MForm-Versionen speicherten Custom-Links im Repeater als Array `['name' => 'Artikelname', 'id' => 'redaxo://12']`, heute ist es ein einzelner Link-String. Eine Konvertierung ist nicht nötig: `MFormOutputHelper::createLinkData()`, `normalizeRepeaterItems()` und `MFormRepeaterItem::url()` lesen beide Formate. Wer MBlock-Daten in den Repeater überführt, nutzt den [Migrationsassistenten](08_mblock_migration.md).
