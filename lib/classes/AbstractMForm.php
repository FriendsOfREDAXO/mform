...
//            echo '</pre>';
     ...
     * @author Joachim Doerr
     */
    public function closeFieldset($value, $attributes = array())
    {
    ...
    */
     ...
     * @author Joachim Doerr
     */
    protected function getGlobalRex()
    {
    ...
    private function getLangData($languageData)
    {
        if (is_array($languageData) === true) {
            ...
            foreach ($languageData as $key => $value) {
                if ($key == $this->REX['LOGIN']->getLanguage() or $key . '_utf8' == $this->REX['LOGIN']->getLanguage()) {
                    $langData = $value;
                }
            }
            if ($langData == '') {
    ...
    private function getRexVars()
    {
        ...
        if ($sliceId != false) {
            $this->getGlobalRex();
            $table = $this->REX['TABLE_PREFIX'].'article_slice';
            $fields = '*';
            ...
            if ($rows > 0) {
                ...
                for ($i = 1; $i <= 20; $i++) {
                    ...
                    if ($i <= 10) {
                        $this->result['filelist'][$i] = $sql->getValue('filelist' . $i);
                        $this->result['linklist'][$i] = $sql->getValue('linklist' . $i);
                        $this->result['file'][$i] = $sql->getValue('file' . $i);
                        $this->result['link'][$i] = $sql->getValue('link' . $i);
                    }

                    if (method_exists('rex_var', 'toArray') === true) {
                        if ($this->isSerial($this->result['value'][$i])) {
                            $result = rex_var::toArray($this->result['value'][$i]);
