    ...
    private function generateLinkElement(MFormItem $item)
    {
        ...
        switch ($item->getType()) {
            ...
            case 'link':
                ...
                $inputValue = ($item->getType() == 'custom-link') ? 'REX_INPUT_VALUE' : $inputValue;
                $html = $class::getWidget($item->getVarId()[0], $inputValue . '[' . $item->getVarId()[0] . ']', $item->getValue(), $item->getParameter());
