        ...
        foreach ($items as $key => $item) {
            ...
            switch ($item->getType()) {
                ...
                case 'close-' . $type:
                    ...
                    if (isset($items[$key + 1]) && $items[$key + 1] instanceof MFormItem && $items[$key + 1]->type != $type || // next item is not from type close group
                        isset($item->getAttributes()["data-close-group-$type"]) && 1 == $item->getAttributes()["data-close-group-$type"]
                    ) {
                        // close group auto
